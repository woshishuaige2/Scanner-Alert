...
ALERT_STRUCTURE_DAMAGE_LOOKBACK_MINUTES = 60  # Recent window used for combined 15s/30s/1m damage analysis
ALERT_STRUCTURE_DAMAGE_MODERATE_PENALTY = 1   # Score penalty for moderate multi-timeframe damage
ALERT_STRUCTURE_DAMAGE_RED_FLAG_PENALTY = 2   # Score penalty for severe multi-timeframe damage
...
SNIPER_PARABOLIC_MAIN_BODY_MULTIPLIER = 2.0      # Latest main-frame candle body must be at least this multiple of the prior candle
SNIPER_PARABOLIC_MAIN_VOLUME_MULTIPLIER = 2.0    # Latest main-frame candle volume must be at least this multiple of the prior candle
SNIPER_PARABOLIC_MAIN_MIN_BODY_PCT = 0.8         # Latest main-frame candle body must still be meaningfully large in absolute percent terms
SNIPER_PARABOLIC_MAIN_MIN_DOLLAR_VOLUME = 50_000 # Absolute dollar-volume floor to avoid low-volume false positives
SNIPER_EXTREME_CLEAN_SIZE_MULTIPLIER = 2.0       # Double the sniper size when the stricter extreme-clean/parabolic filter passes
...
SNIPER_PULLBACK_EXTENSION_FACTOR = 0.20          # Allow entry pullback up to this fraction of extension_pct
...
DYNAMIC_EXIT_INITIAL_DISASTER_STOP_PCT = 5.0     # Initial regular-hours broker stop cushion below actual fill to absorb first-second noise
DYNAMIC_EXIT_INITIAL_DISASTER_STOP_GRACE_SECONDS = 20.0 # Keep the wider initial stop until this hold time unless bounce confirms sooner
DYNAMIC_EXIT_INITIAL_DISASTER_STOP_BOUNCE_PCT = 0.15 # Tighten to structure stop once price has bounced this much from actual fill
DYNAMIC_EXIT_STOP_GUARD_TRIGGER_BUFFER_PCT = 0.20 # Regular-hours bot stop guard only fires this far through the broker stop
DYNAMIC_EXIT_STOP_GUARD_LIMIT_BUFFER_PCT = 1.00   # Extended-hours stop exits cross below live bid/last by this percent
DYNAMIC_EXIT_STOP_GUARD_LIMIT_MIN_DOLLARS = 0.03  # Minimum extended-hours stop-exit limit cushion
DYNAMIC_EXIT_STOP_GUARD_REPRICE_SECONDS = 1.0     # Reprice a pending extended-hours stop exit this often while price remains below stop
...
DYNAMIC_EXIT_VOLUME_FADE_CONFIRM_SECONDS = 8     # Treat the first fade as a warning; require this much extra time before a confirming fade can exit
